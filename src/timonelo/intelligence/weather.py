...
class WeatherIntelligenceEvaluator:
    """Returns sea state, swell, motion risk and sun exposure ONLY when sourced data is supplied.
    ...
    Sea state, swell, motion risk and sun exposure is also a VOLATILE domain: unlike quasi-static ship
    ...
    @staticmethod
    def evaluate(weather_data: Optional[Dict[str, Any]] = None) -> Optional[WeatherIntelligence]:
        if not weather_data:
            return None
        raise NotImplementedError(
            "Sourced weather intelligence is not yet wired to the evidence "
