...
def evidence_verifier(evidence_root: Optional[str] = None) -> SpatialEvidenceVerifier:
    ...
    Only the root is handed over. The verifier builds its own registry on every
    verification, so an artifact registered or deregistered after this call is
    visible to a graph that already exists. Passing a registry -- or a callable
    returning one -- is refused by the verifier: both retain the index they
    were built with.
    """
    root = evidence_root or os.path.join(repo_root(), "evidence", "artifacts")
    return SpatialEvidenceVerifier(root)
